    ...
    foreach ($array as $categoryId => $category) {
        ...
        if ($currentParent == $category['parent']) {                       
           ...
           if($category['status']==1){
                echo '<li><label for="'.$category['menu_id'].'">'.$category['nama'].'</label><input type="checkbox" style="opacity: 0;" name="'.$category['menu_id'].'" value="ON" checked/>';
            }else{
...
?>
   <link rel="stylesheet" type="text/css" href="css/_styles.css" media="screen">
    ...
    $sqlMenu="select group_menu.menu_id from `group` inner join group_menu 
            on(group_menu.group_id=`group`.id_group)
            inner join group_personal 
            on(`group`.id_group=group_personal.group_id)
    ...
    if($arraymMenu!=''){
        ...
        $qry="select menu.*,"
                ...
                . "from menu inner join tampil_form "
                . "on(menu.form_id=tampil_form.form_id)"
                . "where menu.menu_id in('".$listMenu."')";
        ...
        while($row = mysql_fetch_assoc($result)){ 
            ...
            $row['nama'], "status"=>$row['status'], "menu_id"=>$row['menu_id'], "nama_form"=>$row['nama_form']);   
        }
        ?>
        <?php
