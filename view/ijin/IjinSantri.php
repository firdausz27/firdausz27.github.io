...
$cariPersonal=array();
$cariPersonal[] = $personalDao->getPersonal($_SESSION['SES_LOGIN']);
$idSiswa='';
$idJa='';
$tanggal=  date("m/d/Y");
$nomor=0;
...
if($_POST){
    if(isset($_POST['cboPengajar'])){
        ...
        $allJadual=$jadualDao->getJadualByPersonal($idSiswa);
    }
    if(isset($_POST['cboJadual'])){
        $idJa=$_POST['cboJadual'];
        $kajianByJadual = $AddKajianDao->getKajianByPersonal($idJa,"0");
...
<script type="text/javascript">
    function valid(mode){
            ...
            if(txtId==""){
                        ...
                        if(document.getElementById("cboStatus_"+i) && document.getElementById("ket_"+i)){
                            ...
                            //var pilih=document.getElementById("check_"+i).checked;
                                if(status=="blank"){
                                    ...
                                    document.getElementById("cboStatus_"+i).focus();
                                }else if(ket==""){
                ...
                if(hasil==true){
                    if(mode=='insert'){
                        document.forms["form1"].action="?page=IjinAction&action=insert";
                        document.forms["form1"].submit();
                        return true;
                    }else if(mode=='update'){
                        document.forms["form1"].action="?page=IjinAction&action=update";
                        document.forms["form1"].submit();
                        return true;
                    }else{
                        if(confirm('Apakah anda yakin mau menghapus ?')){
                            document.forms["form1"].action="?page=IjinAction&action=delete";
                            document.forms["form1"].submit();
                            return true;
                        }
                    }
   ...
   function kirim(){
            document.forms["form1"].action="?page=IjinSantri";
            document.forms["form1"].submit();
            return true;
   }
...
<form id="form1" name="form1" method="post" action="">
  <table  width="100%" border="0" cellspacing="1" cellpadding="5" class="table-shadow">
    <tr>
        <td colspan="5" align="left" style="background: url(./images/headercontent2.jpg); font-size: 15px; color: #fff;"><!--<img src="./images/form_add.png">-->&nbsp;<b>Pendidikan | Perijinan</b></td>
    </tr>
    <tr>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
