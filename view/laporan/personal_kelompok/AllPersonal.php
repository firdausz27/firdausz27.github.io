...
// $listPerson='';
 ...
 $group=isset($_POST['typeReport']) ? $_POST['typeReport'] : '';
 $groupValue='';
 if($group==1){
     $groupValue=' kategori_pekerjaan in (select distinct kategori_pekerjaan from pekerjaan)';
 }else if($group==2){
     $groupValue=' where year(tgl_lahir) in (select distinct year(tgl_lahir) from personal)';
 }else if($group==3){
     $groupValue=' where tempat_lahir in (select distinct tempat_lahir from personal)';
 }
...
function query($id,$kelamin){
    ...
    while ($dataRow=  mysql_fetch_array($dataQry)){ 
        $personal=new Personal();
          ...
          $personal->setPersonalType($dataRow['personal_type']);
    ...
    <div>
        <table width="650" border="0" cellspacing="1" cellpadding="2">
          ...
          <tr>
            ...
            if($group==1){
                    $groupValue="select distinct kategori_pekerjaan from pekerjaan 
                                inner join personal on(pekerjaan.personal_id=personal.id_siswa)
                                where personal.id_siswa in('".$listSiswa."') ";
                }else if($group==2){
                    $groupValue="select distinct year(tgl_lahir) as tgl from personal where id_siswa in ('".$listSiswa."') order by tgl";
                }else if($group==3){
                    $groupValue="select distinct propinsi,nama_propinsi from personal "
                            . "inner join propinsi on(propinsi.id_propinsi=personal.propinsi)"
                            . " where id_siswa in ('".$listSiswa."') order by propinsi";
                }
            //echo $groupValue;
            $dataFilter  =  mysql_query($groupValue, DBConnection::getConnection())or die("Query error :".  mysql_error());
            while ($dataF=  mysql_fetch_array($dataFilter)){
                if($group==1){
                    echo '<tr><td style="border: 1px solid black; border-top: none;" colspan="9"><i><b>'.$dataF['kategori_pekerjaan'].'</b></i></td></tr>';
                }else if($group==2){
                    echo '<tr><td style="border: 1px solid black; border-top: none;" colspan="9"><i><b>Lahir Tahun '.$dataF['tgl'].'</b></i></td></tr>';
                }else if($group==3){
                    echo '<tr><td style="border: 1px solid black; border-top: none;" colspan="9"><i><b>'.$dataF['nama_propinsi'].'</b></i></td></tr>';
                }
            //ini untuk me loop employee 
                $sql="select distinct * from personal ";
                if($group==1){
                    $sql=$sql." inner join pekerjaan on(personal.id_siswa=pekerjaan.personal_id) where ";
                    $sql=$sql." kategori_pekerjaan ='".$dataF['kategori_pekerjaan']."' ";
                }else if($group==2){
                     $sql=$sql." where year(tgl_lahir)='".$dataF['tgl']."' ";
                }else if($group==3){
                    //$sql=$sql." ".$groupValue;
                    $sql=$sql." where propinsi='".$dataF['propinsi']."' ";
                }
