...
class JadualDao {
    ...
    public function update(Jadual $jadual){
       ...
       $sqlDeletePelajar="delete from jadual_personal where id_jadual='".$jadual->getJadualId()."' and status !=1";
    ...
    public function  getJadualByPersonal($personalId){
        $jaduals=array();
        $sql="SELECT * from jadual 
                inner join jadual_personal
                on(jadual_personal.id_jadual=jadual.id_jadual) 
                where jadual_personal.personal_id ='$personalId' 
                and (jadual.status is null or jadual.status =0)";
        $dataQry  =  mysql_query($sql, $this->connection)or die("Query error :".  mysql_error());
        while ($dataRow=  mysql_fetch_array($dataQry)){  
          $jadual=new Jadual();
          $jadual->setJadualId($dataRow['id_jadual']);
