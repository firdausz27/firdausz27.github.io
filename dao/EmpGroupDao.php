...
class EmpGroupDao {
    ...
    public function insert(Group $group){
        $sql="insert into empgroup (nama_group,tgl_dibuat,keterangan) values 
    ...
    public function update(Group $group){
        $sql="update empgroup set nama_group='".$group->getNamaGroup()."',
    ...
    public function delete(Group $group){
        $sql="delete from empgroup where id_group='".$group->getKode()."'";
        $myQry  =  mysql_query($sql, $this->connection)or die("Error query :".mysql_error());
        return $myQry;
        $this->koneksi->closeConnection();
    }
    ...
    public function getAllGroupPage($hal,$row){
        $groups=array();
        $sql="select * from empgroup order by id_group asc limit $hal,$row";
     ...
     public function getAllGroup(){
        $groups=array();
        $sql="select * from empgroup order by id_group";
        $dataQry  =  mysql_query($sql, $this->connection)or die("Query error :".  mysql_error());
        while ($dataRow=  mysql_fetch_array($dataQry)){  
          $group=new Group();
          $group->setKode($dataRow['id_group']);
    ...
    public function getGroup($id){
        $sql="select * from empgroup where id_group='$id'";
        $dataQry  =  mysql_query($sql, $this->connection)or die("Query error :".  mysql_error());
        while ($dataRow=  mysql_fetch_array($dataQry)){  
          $group=new Group();
          $group->setKode($dataRow['id_group']);
    ...
    public function getCariGroup($field, $text){
        $groups=array();
        $sql="select * from empgroup where $field like '%$text%' order by id_group";
        $dataQry  =  mysql_query($sql, $this->connection)or die("Query error :".  mysql_error());
        while ($dataRow=  mysql_fetch_array($dataQry)){  
          $group=new Group();
          $group->setKode($dataRow['id_group']);
