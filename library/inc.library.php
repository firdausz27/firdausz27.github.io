...
function buatKodeYear($tabel){
	...
	$panjang	= mysql_field_len($struktur,0);
 	...
 	$row	= mysql_fetch_array($qry); 
        ...
        $initialFromDb=substr($row[0],0,strlen($inisial));
        if ($row[0]=="") {
                $angka=0;
        }
        else {
            if($inisial>$initialFromDb){
                $angka=0;
            }else{
                $angka		= substr($row[0], strlen($inisial));
            }
...
function form_tanggal($nama,$value=''){
	echo" <input type='text' name='$nama' id='$nama' size='11' readonly='' maxlength='20' value='$value'/>&nbsp;
	<img src='images/calendar-add-icon.png' align='top' style='cursor:pointer; margin-top:7px;' alt='kalender'onclick=\"displayCalendar(document.getElementById('$nama'),'mm/dd/yyyy',this)\"/>			
	";
}
